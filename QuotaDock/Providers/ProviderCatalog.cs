...
internal static class ProviderCatalog
{
    ...
    [
        new("codex", "Codex Usage", "⌬", "#20E3A2", "请先登录 Codex Desktop，或运行 codex login。", true),
        ...
        new("chatgpt", "ChatGPT Usage", "⌬", "#10A37F", "请先登录 ChatGPT Desktop，或运行 codex login。", true)
    ];
    ...
    public static IReadOnlyList<IQuotaProvider> CreateProviders(HttpClient httpClient)
    {
        return All.Select(definition => (IQuotaProvider)(definition.Id switch
        {
            ...
            "chatgpt" => new ChatGptQuotaProvider(httpClient),
    ...
    public static HashSet<string> Normalize(IEnumerable<string>? ids)
    {
        ...
        var selected = (ids ?? []).Where(known.Contains).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        if (selected.Count == 0)
        {
